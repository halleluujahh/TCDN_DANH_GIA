...
function getItemDisplayText(item) {
  ...
  if (props.isDisplayStatus) {
    return CONSTANTS.STATUS_SHIFT[Number(item.value)];
  }

  // Filter theo cột date
  if (props.typeFilter === CONSTANTS.BASE_INPUT_TYPE.DATE) {
    const dateFilterTypes = [
      Enums.DateFilterColumnType.Different,
      Enums.DateFilterColumnType.Equal,
      Enums.DateFilterColumnType.Less,
      Enums.DateFilterColumnType.LessOrEqual,
      Enums.DateFilterColumnType.Greater,
      Enums.DateFilterColumnType.GreaterOrEqual,
      Enums.DateFilterColumnType.Empty,
      Enums.DateFilterColumnType.NotEmpty,
    ];

    if (dateFilterTypes.includes(item.value)) {
      return CONSTANTS.DATE_FILTER_COLUMN_TYPE_TEXT[Number(item.value)];
    }
    return item.value;
  }
  ...
  const textFilterTypes = [
    Enums.FilterColumnType.Difference,
    Enums.FilterColumnType.Contains,
    Enums.FilterColumnType.NotContains,
    Enums.FilterColumnType.StartsWith,
    Enums.FilterColumnType.EndsWith,
  ];

  if (textFilterTypes.includes(item.value)) {
    return CONSTANTS.FILTER_COLUMN_TYPE_TEXT[Number(item.value)];
  }
      ...
      >
        <div class="dropdown-body-container" style="max-height: 140px">
          <ul class="dropdown-items">
            <li
              v-for="(item, index) in props.comboboxItems"
              :key="index"
              :class="[
                'combobox-item',
                'single',
            ...
            >
              <div class="combobox-item-con view-text">
                <div class="combobox-item--text" v-tooltip="item.value">
